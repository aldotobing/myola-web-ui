...
export default function ApiTestPage() {
  ...
  const [expandedGroups, setExpandedGroups] = useState<Set<string>>(
    new Set(["Data Fetching (GET)", "Admin: Products & Categories"]),
  );
      ...
      <div className="min-h-screen bg-[#0F172A] flex items-center justify-center p-6">
        <div className="bg-[#1E293B] p-10 rounded-3xl max-w-md w-full border border-gray-800 shadow-2xl">
          ...
          <p className="text-gray-400 mb-8 text-center text-sm leading-relaxed">
            This laboratory requires{" "}
            <span className="text-pink-500 font-semibold">Administrator</span>{" "}
            authorization.
          </p>
          <button
    ...
    return (
      <div className="min-h-screen bg-[#0F172A] flex items-center justify-center p-6">
        <div className="bg-[#1E293B] p-10 rounded-3xl max-w-md w-full border border-gray-800 shadow-2xl">
          ...
          <p className="text-gray-400 mb-8 text-center text-sm leading-relaxed">
            Your account role (
            <span className="text-white font-semibold">{user.role}</span>) does
          ...
          <div className="space-y-3">
            ...
            <button
              onClick={() =>
                signOut().then(() =>
                  router.push("/auth/login?redirect=/dev/api-test"),
                )
              }
  ...
  return (
    <div className="flex h-screen bg-[#F8FAFC] text-slate-700 overflow-hidden font-sans selection:bg-pink-100 selection:text-pink-900">
      ...
      >
        ...
        {/* Header */}
        <div
          className={`p-6 border-b border-slate-200 bg-white/50 ${!isSidebarOpen && "flex flex-col items-center px-2"}`}
        >
          <div
            className={`flex items-center ${isSidebarOpen ? "justify-between" : "justify-center"} mb-6`}
          >
            <div className="flex items-center gap-3">
              ...
              {isSidebarOpen && (
                <h1 className="font-extrabold text-slate-900 text-lg tracking-tight whitespace-nowrap">
                  MYOLA API LAB
                </h1>
              )}
            </div>
            {isSidebarOpen && (
              <div className="px-3 py-1 bg-emerald-50 text-emerald-600 rounded-full text-[10px] font-bold border border-emerald-100 tracking-wider whitespace-nowrap">
                CONNECTED
              </div>
            )}
          </div>
            ...
            <div className="mt-4 p-3.5 bg-blue-50 border border-blue-100 rounded-xl">
              <div className="flex items-start gap-3">
                ...
                <p className="text-[11px] text-blue-700 leading-relaxed font-medium">
                  <span className="font-bold">Data Fetching (GET)</span>{" "}
                  endpoints query the database directly. Perfect for retrieving
                  IDs for other tests.
                </p>
              </div>
            </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
          {filteredCollections.map((group, idx) => (
            <div
              key={idx}
              className={`${isSidebarOpen ? "bg-white rounded-2xl border border-slate-200" : "flex flex-col items-center"} overflow-hidden shadow-sm`}
            >
              {isSidebarOpen ? (
                <>
                  <button
                  ...
                  >
                    <div className="flex-1 text-left">
                      <h3 className="text-[11px] font-bold text-slate-900 uppercase tracking-widest truncate max-w-[200px]">
                        {group.group}
                      </h3>
                      {group.description && (
                        <p className="text-[10px] text-slate-500 mt-1 leading-snug font-medium truncate max-w-[220px]">
                          {group.description}
                        </p>
                      )}
                    </div>
                    ...
                    <div className="p-2 space-y-1 bg-slate-50/30">
                      {group.endpoints.map((item, i) => (
                        ...
                        >
                          <div className="flex items-center gap-3">
                            <span
                              className={`text-[10px] font-bold w-12 px-1.5 rounded-lg flex items-center justify-center h-5 flex-shrink-0 tracking-tighter shadow-sm ${
                                item.method === "GET"
                            ...
                            <span
                              className={`text-[12px] font-bold flex-1 leading-tight truncate ${endpoint === item.path ? "text-slate-900" : "text-slate-600 group-hover:text-slate-900"}`}
                            >
                              {item.label}
                            </span>
                          </div>
                ...
                <div className="w-full flex flex-col gap-2">
                  ...
                  <div
                    title={group.group}
                    className="w-10 h-10 rounded-xl bg-white border border-slate-200 flex items-center justify-center text-slate-400 hover:text-pink-600 hover:border-pink-200 cursor-pointer shadow-sm transition-all text-[10px] font-bold"
                  >
        ...
        {/* User Profile */}
        <div
          className={`p-4 bg-white border-t border-slate-200 ${!isSidebarOpen && "flex justify-center"}`}
        >
          <div
            className={`flex items-center gap-3.5 p-3 rounded-2xl bg-slate-50 border border-slate-100 shadow-inner ${!isSidebarOpen && "p-0 bg-transparent border-none shadow-none"}`}
          >
            <div
              className={`w-10 h-10 rounded-xl bg-pink-600 flex items-center justify-center text-white font-bold text-base shadow-lg shadow-pink-100 flex-shrink-0`}
            >
            ...
            {isSidebarOpen && (
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-1.5 mt-0.5">
                  ...
                  <span className="text-[10px] text-pink-600 uppercase font-bold tracking-widest">
                    {user?.role}
                  </span>
                </div>
              </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden relative">
          ...
          <div className="flex gap-4 max-w-6xl mx-auto items-center">
            <div className="relative">
              <select
                ...
                className="appearance-none bg-slate-50 border border-slate-200 rounded-2xl px-8 pr-10 py-4 text-slate-900 font-bold text-sm outline-none focus:border-pink-500 focus:ring-4 focus:ring-pink-50 shadow-sm transition-all cursor-pointer hover:bg-slate-100"
              >
                <option>GET</option>
                <option>POST</option>
                <option>PATCH</option>
            ...
            <button
              onClick={handleSend}
              disabled={isLoading}
              className="bg-pink-600 hover:bg-pink-500 text-white font-bold px-12 py-4 rounded-2xl shadow-xl shadow-pink-100 transition-all flex items-center gap-3 active:scale-[0.98] disabled:bg-slate-200 disabled:text-slate-400 disabled:shadow-none disabled:cursor-not-allowed group"
            >
              {isLoading ? (
        ...
        <div
          className={`flex flex-col bg-slate-50 border-b border-slate-200 transition-all duration-300 ease-in-out ${isRequestPanelExpanded ? "h-[400px]" : "h-[60px]"} flex-shrink-0 overflow-hidden`}
        >
            ...
            <div className="flex gap-10">
              {["params", "authorization", "headers", "body"].map((tab) => (
                <button
                  ...
                  className={`py-5 text-[11px] font-bold uppercase tracking-widest border-b-2 transition-all relative ${activeTab === tab && isRequestPanelExpanded ? "border-pink-600 text-slate-900" : "border-transparent text-slate-400 hover:text-slate-600"}`}
                >
                  {tab}
                  {activeTab === tab && isRequestPanelExpanded && (
                    <span className="absolute bottom-[-2px] left-0 right-0 h-[2px] bg-pink-600 shadow-sm" />
                  )}
                </button>
              ))}
            </div>
          ...
          <div
            className={`flex-1 p-8 overflow-auto custom-scrollbar bg-white ${!isRequestPanelExpanded && "pointer-events-none opacity-0"}`}
          >
            <div className="max-w-6xl mx-auto h-full">
                  ...
                  <div className="p-8 bg-blue-50 border border-blue-100 rounded-3xl flex items-start gap-6 shadow-sm">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-slate-500 leading-relaxed font-medium">
                        Communication with protected endpoints requires a valid
                        Supabase JWT. We handle the handshake automatically
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between px-2">
                      ...
                      <button
                        ...
                        className="text-[10px] text-blue-600 font-bold flex items-center gap-2 hover:bg-blue-100 px-4 py-2 rounded-xl transition-all border border-blue-200 shadow-sm"
                      >
              ...
              {activeTab === "headers" && (
                <div className="space-y-4 max-w-4xl">
                  <div className="flex gap-4 text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-6 px-4">
                  ...
                  {[
                    ...
                    <div key={i} className="flex gap-4 items-center group">
                      <input
                        readOnly
                        value={h.k}
                        className="w-56 bg-white border border-slate-200 rounded-2xl px-5 py-4 text-xs text-slate-900 font-bold shadow-sm group-hover:border-slate-300 transition-colors"
                      />
                      <input
                        readOnly
                        value={h.v}
                        className="flex-1 bg-slate-50 border border-slate-100 rounded-2xl px-5 py-4 text-xs text-slate-400 truncate font-mono font-medium shadow-inner"
                      />
                    </div>
                  ))}
                </div>
              ...
              {activeTab === "params" && (
                <div className="h-full flex flex-col items-center justify-center text-slate-300">
                  ...
                  <h5 className="text-lg font-bold text-slate-900 uppercase tracking-widest">
                    Query Layer
                  </h5>
                  <p className="text-sm mt-3 text-slate-500 font-medium text-center max-w-md">
                    Modify your request parameters directly in the URL bar for
                    real-time validation.
                  </p>
                  <p className="text-xs mt-6 px-6 py-2 bg-slate-100 rounded-full text-slate-400 font-mono italic">
                    Example: /api/admin/products?limit=20&sort=newest
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="flex items-center gap-10">
              ...
              {status && (
                <div className="flex gap-8 items-center">
                  <div
                    className={`px-5 py-2 rounded-xl text-[12px] font-bold shadow-lg tracking-tight ${status < 300 ? "bg-emerald-500 text-white shadow-emerald-100" : "bg-rose-500 text-white shadow-rose-100"}`}
                  >
                    HTTP {status}
                  </div>
            ...
            {response && (
              <div className="flex gap-4">
                <button
                  ...
                  className="text-[11px] font-bold text-slate-600 hover:bg-white transition-all px-6 py-2.5 rounded-xl border border-slate-200 uppercase tracking-widest flex items-center gap-2 shadow-sm"
                >
          ...
          <div className="flex-1 overflow-auto p-8 custom-scrollbar bg-slate-50/30">
            {isLoading ? (
              <div className="h-full flex flex-col items-center justify-center gap-6">
                ...
                <span className="text-[12px] font-bold text-slate-400 uppercase tracking-[0.5em] animate-pulse">
                  Syncing Lab...
                </span>
              </div>
            ) : response ? (
              ...
              <div className="h-full flex flex-col items-center justify-center gap-4 text-slate-400">
                ...
                <p className="text-[11px] font-bold uppercase tracking-[0.4em] opacity-40">
                  Awaiting Signal
                </p>
              </div>
            )}
