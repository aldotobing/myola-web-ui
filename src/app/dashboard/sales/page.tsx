...
export default function SalesDashboard() {
  ...
  const { data: salesProfile, isLoading: isLoadingProfile } = useSWR(
    ...
    async () => {
      ...
      return getSalesProfile(user.id);
    },
  );
  ...
  const { data: summary, isLoading: isLoadingSummary } = useSWR(
    ...
    salesProfile?.id ? () => getCommissionSummary(salesProfile.id) : null,
  );
  ...
  const { data: performance = [], isLoading: isLoadingPerf } = useSWR(
    ...
    salesProfile?.id ? () => getPerformanceData(salesProfile.id) : null,
  );
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
        <div className="bg-white p-8 rounded-3xl shadow-xl border border-gray-100 max-w-md w-full text-center">
          ...
          <p className="text-gray-500 mb-8">
            Hanya Staf Sales yang memiliki akses ke dashboard ini. Silakan
            hubungi Admin jika Anda seharusnya terdaftar sebagai Sales.
          </p>
          <Link
  ...
  const displayProfile =
    salesProfile ||
    (user?.role === "admin" ? { referral_code: "ADMIN-MASTER" } : null);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
            ...
            <p className="text-4xl font-bold text-gray-900">
              Rp{" "}
              {Number(summary?.pending_commission || 0).toLocaleString("id-ID")}
            </p>
            <p className="mt-2 text-gray-400 text-sm font-medium">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 gap-4">
              <Link
                href="/dashboard/sales/members"
                className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between group hover:border-pink-500 transition-all"
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      Lihat siapa saja yang menggunakan kode Anda
                    </p>
                  </div>
                </div>
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      Log transaksi dan status pembayaran komisi
                    </p>
                  </div>
                </div>
                <ChevronRight className="text-gray-300 group-hover:text-pink-500 transition-colors" />
